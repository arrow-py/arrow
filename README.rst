...
#. Run the entire test suite and linting checks by running one of the following commands: ``tox`` (if you have `tox <https://tox.readthedocs.io>`_ installed) **OR** ``make build38 && make test && make lint`` (if you do not have Python 3.8 installed, replace ``build38`` with the latest Python version on your system).
